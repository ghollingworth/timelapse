...
class TimelapseApp:
    ...
    def __init__(self):
        ...
        self.camera_lock = threading.Lock()  # Add camera lock for thread safety
    ...
    def start_timelapse(self):
        ...
        try:
            with self.camera_lock:  # Protect camera initialization
    ...
    def capture_single_image(self):
        ...
        # Prevent single shot while timelapse is running
        if self.is_recording:
            return {"status": "error", "message": "Cannot capture single image while timelapse is running"}
        
        try:
            with self.camera_lock:  # Protect camera operations
    ...
    def update_config(self, updates):
        ...
        if old_hflip != new_hflip or old_vflip != new_vflip:
            try:
                with self.camera_lock:  # Protect camera restart
    ...
    def get_images_list(self, page=1, limit=20):
        """Get paginated list of captured images with metadata."""
    ...
    def get_videos_list(self, page=1, limit=20):
        """Get paginated list of generated videos with metadata."""
    ...
    def delete_all_videos(self):
        ...
        try:
            pattern = os.path.join(VIDEOS_DIR, 'timelapse_*.mp4')
            deleted_count = 0
    ...
    def _capture_worker(self, interval):
        ...
        while self.is_recording:
            try:
                with self.camera_lock:  # Protect camera capture in background thread
...
def get_images():
    ...
    return jsonify(timelapse_app.get_images_list(page=page, limit=limit))
...
def get_videos():
    ...
    return jsonify(timelapse_app.get_videos_list(page=page, limit=limit))
...
def delete_videos_manual():
    """Manually delete all videos."""
    return jsonify(timelapse_app.delete_all_videos())
